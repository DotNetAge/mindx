...
// IndexFile indexes a single file, handles old-chunk cleanup and cache persistence.
//
// This is intended for files managed outside the normal Sync/SyncFiles flow,
// such as .README.md files generated by RegionIndexer. It:
//  1. Removes previously indexed chunks for the same relPath (if cached)
//  2. Indexes the new file content
//  3. Updates the in-memory cache
//  4. Persists the cache to disk
//
// absDir is the project root directory, absPath is the full path to the file.
func (p *IndexService) IndexFile(ctx context.Context, absDir, absPath string) error {
	...
	if existing := p.cache.Get(relPath); existing != nil && len(existing.Chunks) > 0 {
		p.removeChunks(ctx, existing.Chunks)
	}

	// ── Index new content ──
	fileCtx := goragindexer.WithRegionID(ctx, goragindexer.RegionIDFromDir(absDir))
	chunks, err := p.indexFile(fileCtx, absPath)
	if err != nil {
		return err
	}
	...
	if len(chunks) > 0 {
		...
		// No chunks produced for this file
		p.cache.Delete(relPath)
	}

	// ── Persist cache ──
	if saveErr := p.cache.SaveToFile(p.cacheDir); saveErr != nil && p.logger != nil {
		p.logger.Warn("index-service: failed to save cache after IndexFile", "error", saveErr)
	}
