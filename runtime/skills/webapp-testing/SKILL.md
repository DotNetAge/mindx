...
**务必先用 `--help` 运行脚本**查看用法。确认需要自定义方案之前，不要阅读源码。这些脚本体积较大，会占用大量上下文窗口。它们设计为黑盒脚本，直接调用即可，无需加载到上下文窗口中。
...
- **将内置脚本作为黑盒使用** — 要完成任务时，看看 `scripts/` 中的脚本能否帮上忙。这些脚本能可靠处理常见的复杂工作流，不会占用上下文窗口。用 `--help` 查看用法，直接调用即可。
