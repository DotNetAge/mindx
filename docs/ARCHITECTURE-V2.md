...
Transcript 模式下，消息按时间顺序排列，添加编号和时间戳：
...
借鉴 Gemini CLI 风格的布局：左侧 ASCII 标识 + 右侧状态信息，下方展示通知消息。
...
│  ───────────────────────────────────────────────────────────────────────────────────────  │
│                                                                                          │
│  ℹ Session loaded: a1b2c3d4                                                             │
│  ℹ 2 agents available (use @ to switch)                                                 │
│                                                                                          │
└──────────────────────────────────────────────────────────────────────────────────────────┘
```

规则:
  • 欢迎面板仅在应用首次启动、无已有会话时展示一次
  • 显示后不再重复出现（由 WelcomeShown bool 控制）
  • 由**标识区**和**信息面板**两部分组成，标识区在左，信息面板在右

标识区:
  • 使用 MindX ASCII 艺术字（MindxLogo()），固定 6 行
  • 标识颜色：品牌青色（#4FC3F7）

信息面板:
  • 位于标识区右侧，左对齐排列
  • 第一行：`MindX CLI v{version}` — 白色加粗
  • 第二行：`Authenticated as {agentName}` — 绿色（#4CAF50），表示登录状态
  • 第三行：`Workspace: {path}` — 白色
  • 第四行：`Session: {id}` — 白色
  • 第五行：`Agent: {name} ({role})` — 白色
  • 第六行：`Model: {modelName}` — 白色

底部通知:
  • 信息面板下方以一条全宽分割线（`─`）隔开
  • 分割线使用 ThemeDimColor 显示（#666666）
  • 通知条目以 `ℹ` 图标开头，灰色（#AAAAAA）
  • 通知内容：会话加载、Agent 可用数、版本提示等启动信息
  • 每条通知占一行，多条依次排列，结束后留空行
...
│  • Markdown 渲染（glamour）                                  │
│  • 搜索匹配高亮                                              │
│  • Transcript / Normal / Fullscreen 三种模式                 │
...
│  - ViewMode     Normal/Transcript/Full                      │
│  - SearchState  (当前搜索位置)                               │
│  - WelcomeShown bool                                         │
...
│   │   ├── welcome.go     # 欢迎面板渲染
│   │   └── viewmode.go    # Normal/Transcript/Fullscreen 视图
│   │
│   ├── statusbar/
...
V1 代码中的 setter 模式、`AgentAnswer` 数据行为耦合、`string contentType` 路由等问题深植于所有组件中，
渐进式重构无法根除这些架构缺陷，新旧代码共存只会相互干扰。

**唯一正确的做法：彻底删除旧 UI，从零重新实现。**

```
保留层（非 UI 代码，与 TUI 无关，完全不动）：
  ────────────────────────
  • internal/core/        — App 编排、配置、会话元数据
  • internal/commands/    — 命令注册
  • pkg/                  — 日志、存储、调度等通用库
  • internal/setup/       — 安装向导
  • internal/svc/         — 守护进程
  • cmd/                  — CLI 入口
...
重写层（全部删除，不留任何旧代码）：
  ────────────────────────
  • internal/client/      — 整个 TUI 层全部删除
                          按 §11 目录结构，从 §12 给定的顺序从零新建

重写顺序：
  ──────────
  ① client.go     — rootModel 骨架 + NewProgram
  ② types.go      — 强类型消息定义（§4）
  ③ data/         — 纯数据结构 AnswerData / ActionStep / ResultEntry 等（§3）
  ④ style/        — 主题色 + 组件样式
  ⑤ component/    — 按依赖顺序逐个实现：
     a) StatusBar         — 展示型，无外部依赖
     b) NotificationBar   — 浮动型，独立
     c) InputArea         — 交互型，依赖 textarea
     d) AgentSuggestion + CommandSuggestion — 子组件
     e) ChoicesPanel      — 模态型
     f) ConversationPanel — 容器型，依赖所有数据层
     g) Welcome           — 欢迎面板渲染
  ⑥ render/       — Markdown / 表格 / 待办渲染器
  ⑦ 组装：rootModel.Update + rootModel.View
  ⑧ command/ + session/ — 命令注册与会话持久化
```
